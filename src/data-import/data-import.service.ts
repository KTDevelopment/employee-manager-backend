...
export class DataImportService {
  ...
  private async loadEmployeesWithProjects(): Promise<Employee[]> {
    const plainEmployees = await this.httpService.get(this.configService.get('dataSource.employeesBaseUrL')).toPromise();
    return await Promise.all(
      plainEmployees.data.map(async (single: APIEmployeeDTO) => {
  ...
  private async getProjects(url: string): Promise<Project[]> {
    ...
    return await Promise.all(
      plainProjects.map(async (single: APIProjectDTO) => {
